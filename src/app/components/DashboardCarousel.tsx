...
export default function DashboardCarousel() {
  ...
  return (
    <section className="w-full max-w-2xl mx-auto py-10 px-2 rounded-2xl bg-gradient-to-b from-slate-950 via-blue-200/90 to-blue-50/90 dark:from-[#1e293b] dark:via-[#23272f] dark:to-[#181e29] shadow-lg dark:border-gray-800 mt-8">
      <h2 className="text-2xl font-bold mb-6 text-center text-gray-100 dark:text-white">{language === 'es' ? 'Dashboard - Detalle' : 'Dashboard - Details'}</h2>
      <Swiper
        spaceBetween={24}
        slidesPerView={1}
        ...
        {dashboardSlides.map((slide, idx) => (
          <SwiperSlide key={idx}>
            <div className="rounded-xl shadow-lg border bg-white dark:bg-[#23272f] border-gray-200 dark:border-gray-700 overflow-hidden">
              ...
              <div className="p-4">
                <h3 className="text-lg font-semibold mb-2 text-gray-900 dark:text-gray-100">{slide.title[language]}</h3>
                <p className="text-base text-gray-700 dark:text-gray-200">{slide.desc[language]}</p>
              </div>
            </div>
          </SwiperSlide>
        ))}
